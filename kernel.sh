...
[[ ! -d $(pwd)/tc-clang ]] && git clone https://github.com/fadlyas07/clang-11.0.0 --depth=1 -b master tc-clang
...
if ! [[ -f "$kernel_img" ]]; then
    ...
    tg_sendlog=$(echo https://del.dog/$(jq -r .key <<< $(curl -sf --data-binary "$(cat $(echo $TEMP/*.log))" https://del.dog/documents)))
